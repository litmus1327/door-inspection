...
interface ProjectSetupPanelProps {
  ...
  onSave: (setup: ProjectSetup) => void; // called with configured:true on Finish
...
export default function ProjectSetupPanel({ initial, onSave, onCancel }: ProjectSetupPanelProps) {
    ...
    <div className="fixed inset-0 z-40 bg-black/40 flex items-end sm:items-center justify-center p-3">
      <div className="w-full sm:max-w-md bg-card border border-border rounded-xl shadow-2xl overflow-hidden">
        <div className="px-4 py-3 border-b border-border">
          <h2 className="text-sm font-bold tracking-tight">Project setup</h2>
          <p className="text-xs text-muted-foreground mt-1">Answer these once for this project. They shape every inspection. Required before dropping pins.</p>
        </div>
        ...
        <div className="px-4 py-3 border-t border-border flex gap-2">
          ...
          <button
            onClick={() => onSave({ ...s, configured: true })}
